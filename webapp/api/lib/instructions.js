...
7. Write the suggested job duty text as tightly as real ETA-9141 filings do, the way an employer's own filing reads, not a generic HR job description. Compress it into one dense sentence (two at most), joining duty clusters with commas and semicolons rather than writing a separate full sentence per duty. Do not open with a generic preamble sentence like "Landscape and maintain grounds of residential and commercial properties." Fold every tool/equipment name into a single closing clause introduced by "using," rather than giving tools their own sentence. Minimize total word count while still including every duty and tool that passed the rules above; every word must earn its place. Target this density (structure only, content will differ by SOC): "Mow, trim, and plant; water, dig, and weed planting beds; lay sod; mulch beds, using mowers, string trimmers, hedge trimmers, blowers, shovels, rakes, and sprinkler/irrigation equipment."
